...
const App = () => {
	...
	const [ licenseKey, setLicenseKey ] = useState(
		window.SaasMenu_Vars?.license?.key || ''
	);
	const [ isActivatingLicense, setIsActivatingLicense ] = useState( false );
	const [ isPro, setIsPro ] = useState(
		window.SaasMenu_Vars?.is_pro || false
	);
	...
	const handleSaveSettings = async () => {
		...
		try {
			...
			if ( response.success ) {
				toast.success(
					__( 'Settings saved successfully!', 'saasmenu' )
				);
				...
				toast.error(
					response.message ||
						__( 'Failed to save settings.', 'saasmenu' )
				);
			}
		} catch ( err ) {
			toast.error(
				err.message ||
					__( 'Connection error. Please try again.', 'saasmenu' )
			);
		} finally {
			setIsSaving( false );
		}
		...
		setTimeout( () => {
			...
			if (
				...
				if ( window.SaasMenu_Vars ) {
					...
					window.SaasMenu_Vars.settings.license_tier =
					...
					<div className="bg-slate-50 p-8 md:w-1/3 flex flex-col justify-between border-r border-slate-200">
						...
						<div className="space-y-3 mt-8">
							{ [
								{
									step: 1,
									label: __( 'Design Theme', 'saasmenu' ),
								},
								{
									step: 2,
									label: __( 'Brand Identity', 'saasmenu' ),
								},
								{
									step: 3,
									label: __( 'All Done', 'saasmenu' ),
								},
							].map( ( s ) => (
								<div
									key={ s.step }
				...
				<div className="flex items-center gap-3">
					...
					<div>
						...
						<span className="text-xs text-slate-400">
							{ sprintf(
								__( 'Version %s', 'saasmenu' ),
								window.SaasMenu_Vars?.version || '2.0.0'
							) }
						</span>
					</div>
				</div>
				...
				<div className="flex-1 flex flex-col bg-white overflow-y-auto">
					...
					<div className="flex border-b border-slate-100 bg-slate-50/50 px-6 overflow-x-auto shrink-0">
						{ [
							{
								id: 'templates',
								label: __( 'Templates', 'saasmenu' ),
								icon: <Layers className="h-4 w-4" />,
							},
							{
								id: 'colors',
								label: __( 'Colors & Sizing', 'saasmenu' ),
								icon: <Palette className="h-4 w-4" />,
							},
							{
								id: 'roles',
								label: __( 'Role Visibility', 'saasmenu' ),
								icon: <UserCheck className="h-4 w-4" />,
							},
							{
								id: 'advanced',
								label: __( 'Advanced', 'saasmenu' ),
								icon: <Sliders className="h-4 w-4" />,
							},
							{
								id: 'license',
								label: __( 'License', 'saasmenu' ),
