        ...
        <div className="saasmenu-color-customizer p-6 grid grid-cols-1 lg:grid-cols-2 gap-8 bg-white">
            ...
            <div className="space-y-6">
                <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1">
                        { __( 'Configure brand identity colors, sizing parameters, and logo settings.', 'saasmenu' ) }
                    </p>
                </div>
                ...
                <div className="space-y-2 border-b border-slate-100 pb-5">
                    ...
                    { ! contrastPass && (
                        <div className="mt-3 p-3 bg-amber-50 border border-amber-200 rounded-lg flex items-start gap-2 text-amber-800 text-xs">
                            ...
                            <div>
                                <span className="font-semibold">{ __( 'WCAG Contrast Warning: ', 'saasmenu' ) }</span>
                                { sprintf(
                                    __( 'Selected brand color contrast ratio is %s:1 against default template text, which fails WCAG 2.1 AA requirements (4.5:1). Text visibility may be limited.', 'saasmenu' ),
                                    contrastRatio
                                ) }
                            </div>
                        </div>
                ...
                <div className="space-y-2 border-b border-slate-100 pb-5">
                    ...
                    <div className="space-y-3">
                        ...
                        <p className="text-[11px] text-slate-400">
                            { __( 'Max recommended size: 200px wide by 60px high. SVG or transparent PNG preferred.', 'saasmenu' ) }
                        </p>
                    </div>
                </div>
                ...
                <div className="space-y-4 border-b border-slate-100 pb-5">
                    ...
                    <div className="space-y-1">
                        <div className="flex justify-between text-xs font-medium text-slate-500">
                            <span>{ __( 'Expanded Sidebar Width', 'saasmenu' ) }</span>
                            <span className="font-mono text-slate-700">{ settings.sidebarWidth }px</span>
                        </div>
                        <input
                            type="range"
                            min="200"
                    ...
                    <div className="space-y-1">
                        <span className="text-xs font-medium text-slate-500">{ __( 'Menu Spacing Density', 'saasmenu' ) }</span>
                        <div className="grid grid-cols-3 gap-2 mt-1.5">
                            { [
                                { value: 'compact', label: __( 'Compact', 'saasmenu' ) },
                                { value: 'normal', label: __( 'Normal', 'saasmenu' ) },
                                { value: 'relaxed', label: __( 'Relaxed', 'saasmenu' ) },
                            ].map( ( d ) => (
                                <button
                                    key={ d.value }
                                    type="button"
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-1">
                        <div className="flex justify-between text-xs font-medium text-slate-500">
                            ...
                            <span className="font-mono text-slate-700">{ settings.topBarHeight }px</span>
                        </div>
                        <input
                            type="range"
                            min="32"
            ...
            <div className="border border-slate-100 bg-slate-50/50 p-6 rounded-xl space-y-4">
                ...
                <ul className="text-xs text-slate-600 space-y-3 pl-4 list-disc leading-relaxed">
                    <li>
                        <strong className="text-slate-700">{ __( 'Contrast Compliance: ', 'saasmenu' ) }</strong>
                        { __( 'Ensure your brand color passes contrast compliance with the text color of your active design template. This secures compliance under WCAG 2.1 AA requirements, preventing readability barriers.', 'saasmenu' ) }
                    </li>
                    <li>
                        <strong className="text-slate-700">{ __( 'Dynamic Custom Properties: ', 'saasmenu' ) }</strong>
                        { __( 'Once saved, your color specifications generate root CSS custom properties in the WordPress header. Changing templates retains color specifications.', 'saasmenu' ) }
                    </li>
                    <li>
                        <strong className="text-slate-700">{ __( 'Width Adjustment: ', 'saasmenu' ) }</strong>
                        { __( 'Set sidebar widths between 200px and 320px depending on the depth of your site menu titles to avoid truncation.', 'saasmenu' ) }
                    </li>
                    <li>
                        <strong className="text-slate-700">{ __( 'Collapsing state: ', 'saasmenu' ) }</strong>
                        { __( 'Folding the sidebar reduces layout spacing into a clean, icon-only 60px panel automatically.', 'saasmenu' ) }
                    </li>
                </ul>
            </div>
        </div>
