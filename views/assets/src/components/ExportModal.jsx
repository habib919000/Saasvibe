...
import { Download, Upload, ShieldAlert, X, CheckCircle } from 'lucide-react';
		...
		reader.onload = async ( event ) => {
			try {
				...
				if ( response.success ) {
					...
					setErrorMessage(
						response.message ||
							__( 'Import failed. Unknown error.', 'saasmenu' )
					);
				}
			} catch ( err ) {
				setImportStatus( 'error' );
				setErrorMessage(
					__( 'Invalid JSON file content.', 'saasmenu' )
				);
			} finally {
				...
				<div className="p-6 space-y-6">
					...
					<div className="grid grid-cols-2 gap-4">
						...
						>
							...
							/>
							<span className="text-xs font-semibold">
								{ __( 'Import JSON', 'saasmenu' ) }
							</span>
							<span className="text-[10px] text-slate-400 text-center leading-normal">
								{ __(
									'Upload saasmenu-settings.json file.',
									'saasmenu'
								) }
							</span>
						</button>
					...
					{ isImporting && (
						<div className="flex items-center justify-center gap-2 p-3 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-600">
							...
							{ __(
								'Uploading and writing configurations…',
								'saasmenu'
							) }
						</div>
					) }
					...
					{ importStatus === 'success' && (
						<div className="p-3 bg-green-50 border border-green-200 rounded-lg flex items-center gap-2 text-green-800 text-xs">
							...
							<span>
								{ __(
									'Settings imported successfully! Reloading variables…',
									'saasmenu'
								) }
							</span>
						</div>
