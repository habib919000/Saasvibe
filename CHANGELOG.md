...
### Changed
- **Default sidebar width is now 200px**: A fresh activation started at 240px, wider than most sites need and wider than the 160px WordPress default. New installs now start at 200px. Existing sites keep whatever width they saved — only the activation default and the CSS fallbacks moved.
- **Custom site logo moved to the toolbar**: The logo was injected at the top of the sidebar, above the menu. It now replaces the house glyph on the toolbar's site-name node, where WordPress already puts the site's identity, and is offered in two shapes: **Logo icon**, a square mark beside the site name, and **Full logo**, a wordmark on its own with the site name hidden since the wordmark already carries it. The height follows the top bar setting, floored so a short bar cannot shrink it away.
...
- **Doubled gutter between menu icons and labels**: WordPress puts a label 36px from the row's content edge — a 36px icon box with the glyph centred in it, so the only space after the 20px glyph is the 8px left over inside the box. The density shorthand added its horizontal value on top of that box, pushing labels to 48px and leaving a 20px gap, which cost 12px of label width at every density and wrapped longer menu names early. The leading padding is dropped so labels sit at core's 36px; density still drives the vertical rhythm, and submenu labels keep lining up under their parent's.
